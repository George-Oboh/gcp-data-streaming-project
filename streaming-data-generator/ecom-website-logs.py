...
#cluster_name = "gs://spark-rtp" # cluster in which kafka & zookeeper are installed  
...
user_agent_list = [faker.firefox, faker.chrome, faker.safari, faker.internet_explorer, faker.opera]
...
while (flag):
    ...
    vrb = numpy.random.choice(http_verb,p=[0.6,0.1,0.1,0.2]) #Generate a non-uniform random sample from http_verb array
    ...
    resp = numpy.random.choice(response,p=[0.9,0.04,0.02,0.04]) #Generate a non-uniform random sample from response array
    byt = int(random.gauss(5000,50))
    referer = faker.uri()
    useragent = numpy.random.choice(user_agent_list,p=[0.5,0.3,0.1,0.05,0.05] )() #Generate a non-uniform random sample from user_agent_list
    
    #Creating a Kafka producer to stream data output to kafka
    producer = KafkaProducer(bootstrap_servers=['spark-rtp-w-0:9092'],value_serializer=lambda v: json.dumps(v).encode('utf-8'))
    ...
    json_str = {"date_time": date_time_format, "state":fake_state, "ip_address":ip, "category":uri_segments[0],'sub_cat':uri_segments[1],'type':uri_segments[2],"pid":pid}
