...
bucket_name = "gs://website-data-streams"
...
#Creating a time segment window where stream data within 3 minutes of datetime is added
#Use withWatermark to handle late data arrival
df_time_segment_window = df_streaming_visits \
...
#Create a function to write output data in avro format
def foreach_batch_function(df,epoch_id) :

    #Splitting the time window column to start_time and end_time
    ...
    .option("path",bucket_name+"/final_aggregated_data/") \
    .save()

query = (

        df_time_segment_window.writeStream.trigger(processingTime="60 seconds") \
